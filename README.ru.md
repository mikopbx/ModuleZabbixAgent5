...
| Метрика | Ключ айтема Zabbix |
|---|---|
| Топ-10 процессов по памяти | `asterisk[topMem]` |
| Топ-10 процессов по CPU | `asterisk[topCpu]` |

`asterisk[topMem]` возвращает до 10 текстовых строк вида `RSS_МБ PID КОМАНДА`,
отсортированных по убыванию резидентной памяти; потоки ядра исключаются.

`asterisk[topCpu]` возвращает до 10 текстовых строк вида `CPU% PID КОМАНДА`,
отсортированных по убыванию. Загрузка измеряется за 1-секундный интервал в момент
опроса, 100% соответствует суммарной мощности всех ядер процессора.

`КОМАНДА` — это полная командная строка, обрезанная до 120 символов, поэтому
PHP-воркер показывает свой заголовок процесса (например,
`MikoPBX\Core\Workers\WorkerCallEvents`), а не просто `php`. Команда идёт
последней, так как может содержать пробелы. Для потоков ядра, у которых
командной строки нет, выводится имя ядра в квадратных скобках.
...
- MikoPBX 2025.1.1+
- PHP 8.4
- Zabbix Server 6.4+ (шаблон в формате экспорта 6.4, импортируется в 6.4, 7.0 и новее)
