...
`asterisk[topMem]` возвращает до 10 текстовых строк вида `RSS_МБ ИМЯ PID`,
отсортированных по убыванию резидентной памяти; потоки ядра исключаются.

`asterisk[topCpu]` возвращает до 10 текстовых строк вида `CPU% ИМЯ PID`,
...
Пробелы в имени процесса в обеих метриках заменяются на подчёркивания, чтобы
в каждой строке оставалось ровно три поля.
